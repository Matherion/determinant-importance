...
unsortedDetImpObject <-
  determinantImportance(dat,
                        ...
                        drawPlot=FALSE);
unsortedDetImp <- unsortedDetImpObject$output$plot;
sortedDetImp <-
  determinantImportance(dat,
                        determinants=unlist(detStruct$intention$attitude$Likelihood$varNames),
                        targets=c(detStruct$intention$attitude$scaleVarName,
...
ggsave(file.path(outputPath, "determinant importance (unsorted).png"),
     plot=unsortedDetImp,
     width=attr(unsortedDetImp, 'width'),
     height=attr(unsortedDetImp, 'height'),
     dpi=300, type="cairo");
ggsave(file.path(outputPath, "determinant importance (sorted).png"),
     plot=sortedDetImp,
     width=attr(sortedDetImp, 'width'),
     height=attr(sortedDetImp, 'height'),
     dpi=300, type="cairo");

```

```{r fig.cap="Unsorted determinant important diamond plot", fig.width=attr(unsortedDetImp, 'width'), fig.height=attr(unsortedDetImp, 'height')}
  grid.newpage();
  grid.draw(unsortedDetImp);
```

```{r fig.cap="Sorted determinant important diamond plot", fig.width=attr(sortedDetImp, 'width'), fig.height=attr(sortedDetImp, 'height')}
  grid.newpage();
  grid.draw(sortedDetImp);
```
